...
- 应用使用 AirPods 麦克风时自动进入通话音频，结束通话后恢复高音质播放；
...
在设置中开启“自动管理 AirPods 通话模式和高音质模式”后，PodRelay 会把所选 AirPods 麦克风设为 Windows 默认输入，并观察该麦克风的真实音频会话：

- Windows 11 使用统一的蓝牙输出端点；按照[微软的经典蓝牙音频说明](https://learn.microsoft.com/windows-hardware/drivers/bluetooth/bluetooth-classic-audio)，麦克风被应用打开时由 Windows 自动切到 HFP，关闭后自动回到 A2DP；
- Windows 10 通常分别暴露 Stereo 与 Hands-Free 输出；PodRelay 在麦克风会话活动时选择 Hands-Free，释放约 2 秒后恢复 Stereo；
- 切换依据是会议、语音聊天或录音应用是否仍占用麦克风，不是你每句话之间有没有声音；
- 2 秒缓冲用于容忍 Teams、Discord、游戏语音等应用短暂重建会话，避免输出模式来回跳动。

经典蓝牙的物理限制仍然存在：A2DP 提供高质量立体声但不承载麦克风；使用 AirPods 麦克风时必须进入 HFP，播放质量会暂时下降。结束麦克风会话后才会恢复高音质。Bluetooth LE Audio 设备可能有不同能力，但 AirPods 的经典蓝牙路径仍受此限制。
...
- 经典蓝牙无法同时提供 A2DP 高质量立体声和 HFP 麦克风；自动切换以应用占用麦克风的整个会话为单位；
...
dotnet run --project ./src/PodRelay.Diagnostics -- capture-audio
