...
3. **Audio** enumerates Core Audio render and capture endpoints, selects the target from Bluetooth profile and form-factor metadata, observes active microphone sessions, applies output routing, and sends distinct Windows `APPCOMMAND_MEDIA_PAUSE` / `APPCOMMAND_MEDIA_PLAY` commands for opt-in in-ear control. This supports both Windows 10's separate A2DP/HFP endpoints and Windows 11's unified render endpoint without relying on localized friendly names.
...
The two public in-ear bits are retained independently. Before those bits can control media, the advertisement model must match the byte-swapped Apple product ID from the selected Windows Bluetooth device (for example Windows `PID&2024` maps to advertisement model `0x2420`). Drivers that hide container hardware IDs retain the conservative family-level fallback. A 180 ms stability delay filters rapid status chatter while keeping the response close to the AirPods advertisement cadence. A decrease in the worn-earbud count sends the non-toggling pause command only when the target AirPods connection was verified recently. Reinsertion sends the distinct play command only to the same still-valid window that received PodRelay's pause. After a successful resume, a three-second settling window ignores a partial one-pod sensor fallback but never ignores a transition to zero worn pods. This avoids both nearby cross-variant frames, post-insertion pause bounce, and the dangerous `PLAY_PAUSE` toggle, which could start an originally paused video when an earbud is removed. The packaged-media-session API was intentionally rejected because it returns access denied for this unpackaged desktop deployment.
...
Automatic call audio is based on Core Audio session lifetime rather than voice activity. Microsoft's [`IAudioSessionControl.GetState`](https://learn.microsoft.com/windows/win32/api/audiopolicy/nf-audiopolicy-iaudiosessioncontrol-getstate) contract reports a capture session active while at least one stream is running, including natural pauses in speech. On Windows 10 split topology, a pure policy selects the Hands-Free render endpoint during that session and restores A2DP after a two-second release grace period. On Windows 11 unified topology, PodRelay does not fight the operating system: Windows selects HFP while the input endpoint is open and returns to A2DP after it closes. PodRelay only keeps the selected AirPods capture endpoint as the default and records diagnostics.
